const tableBody = document.querySelector(".available-rooms-info-table-body");
...
async function fetchBookings() {
  ...
  bookingsInfo.forEach((booking) => {
    const stylesForBookingStatus = function () {
      if (booking.bookingDetails.status.toLowerCase() === "booked") {
        return "style=color:lightgreen";
      } else if (booking.bookingDetails.status.toLowerCase() === "cancelled") {
        return "style=color:red";
      } else if (booking.bookingDetails.status.toLowerCase() === "pending") {
        return "style=color:lightblue";
      } else if (booking.bookingDetails.status.toLowerCase() === "checkout") {
    ...
    const template = `<tr>
                    ...
                    <td ${stylesForBookingStatus(booking.status)} >${
